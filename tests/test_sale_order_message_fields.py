...
Both select the same MercadoLibre order: ``meli_order`` is itself computed as
``meli_orders[0]`` (``_get_meli_order``), so the two mechanisms differ in form,
not in outcome. These tests therefore assert BEHAVIOUR only — never `related`
vs `compute`, never the name of a compute method — so the exact same file keeps
its meaning once the upstream implementation replaces ours.
...
runtime introspection can see that (``_fields`` holds a single entry) and every
behaviour test above still passes, so the guard parses the module with ``ast``.
"""
